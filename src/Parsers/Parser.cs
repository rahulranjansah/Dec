...
namespace Parser
{
    ...
    public static class Parser
    {
        ...
        public static void ParseStmtList(List<string> lines, BlockStmt stmt)
        {
            ...
            if (lines.Count == 0) { throw new ParseException("Block stmt is empty");}
            ParseStmtListHelper(content, stmt, 0);
        }

        public static void ParseStmtListHelper(List<Tokenizer.Token> content, BlockStmt stmt, int i)
        {
            if (content[i]._tkntype == TokenType.LEFT_CURLY && content[content.Count - 1]._tkntype == TokenType.RIGHT_CURLY)
            {
                ParseStatement(content.GetRange(i+1, content.Count - i - 1), stmt);
            }

            ParseStmtListHelper(content.GetRange(i, content.Count - i - 1), stmt, i+1);
        }

        public static AST.BlockStmt ParseBlockStmt(List<string> lines, SymbolTable<string, object> keyval)
        {
            return new AST.BlockStmt(ParseStmtList(lines, keyval));
